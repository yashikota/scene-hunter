...
func TestRealChrono_Now_Properties(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"unix timestamp":          {func(t *testing.T) { chronoProvider := chrono.New(); now := chronoProvider.Now(); unix := now.Unix(); if unix <= 0 { t.Errorf("Unix timestamp %d should be positive", unix) }; if unix < 1577836800 { t.Errorf("Unix timestamp %d is too old (before 2020)", unix) }; if unix > 4102444800 { t.Errorf("Unix timestamp %d is too new (after 2100)", unix) } }},
		"weekday":                 {func(t *testing.T) { chronoProvider := chrono.New(); now := chronoProvider.Now(); weekday := now.Weekday(); if weekday < time.Sunday || weekday > time.Saturday { t.Errorf("Invalid weekday: %v", weekday) } }},
		"year month day":          {func(t *testing.T) { chronoProvider := chrono.New(); now := chronoProvider.Now(); year, month, day := now.Date(); if year < 2020 || year > 2100 { t.Errorf("Year %d is out of expected range", year) }; if month < time.January || month > time.December { t.Errorf("Invalid month: %v", month) }; if day < 1 || day > 31 { t.Errorf("Invalid day: %d", day) } }},
