...
// ファイルアップロード
app.post("/upload", zValidator("form", imageFormSchema), async (c) => {
  try {
    ...
    if (!imageFile || !(imageFile instanceof File)) {
      return c.text("画像ファイルが必要です", 400);
    }
    if (!filename || typeof filename !== "string") {
      return c.text("ファイル名が必要です", 400);
    }

    // バリデーション
    if (!imageFile.type.startsWith("image/")) {
      return c.text(
        "無効なファイル形式です。画像ファイルを送信してください",
        400,
      );
    }

    // パラメータ取得
    const width = formData.get("w") ? Number(formData.get("w")) : DEFAULT_WIDTH;
    ...
    // 最適化に失敗した場合
    if (!optimizedImage) {
      return c.text("画像の最適化に失敗しました", 500);
    }
    ...
    try {
      ...
      return c.text("ファイルのアップロード中にエラーが発生しました", 500);
    }
    ...
    return c.text("画像処理中にエラーが発生しました", 500);
  }
});
...
app.delete("/file/*", async (c) => {
  try {
    ...
    return c.text("ファイルを削除しました", 200);
  } catch (error) {
    console.error("File deletion error:", error);
    return c.text("ファイル削除中にエラーが発生しました", 500);
  }
});
...
app.delete("/bucket/*", async (c) => {
  try {
    ...
    if (objects.objects.length === 0) {
      return c.text("指定されたバケットが空か存在しません", 404);
    }
    ...
    return c.text(
      `${bucketPrefix} バケット内のファイルを全て削除しました`,
      200,
    );
  } catch (error) {
    console.error("Bucket deletion error:", error);
    return c.text("バケット削除中にエラーが発生しました", 500);
  }
});
...
app.get("/file/*", async (c) => {
  try {
    ...
    return c.text("ファイル取得中にエラーが発生しました", 500);
  }
});
