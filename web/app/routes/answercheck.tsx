...
export default function AnswerCheckPage() {
  const navigate = useNavigate();
  const [players, setPlayers] = useState<PlayerResult[]>([]);
  const [gmImageUrl, setGmImageUrl] = useState<string>("");
  const [loading, setLoading] = useState(true);
  ...
  const handleComplete = () => {
    ...
    else if (roundNumber === 2) navigate("/interimresult2");
    else if (roundNumber === 3) navigate("/finalresult");
    else navigate("/");
  };
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [roundRes, resultRes] = await Promise.all([
          fetch(`https://scene-hunter.yashikota.com/api/rooms/${roomId}/rounds/${roundId}`),
          fetch(`https://scene-hunter.yashikota.com/api/rooms/${roomId}/rounds/${roundId}/results`),
        ]);
        ...
        // GM画像
        setGmImageUrl(`https://scene-hunter-image.yashikota.workers.dev/file/${roundData.master_photo_id}`);
  
        // プレイヤー結果統合
        const merged: PlayerResult[] = resultData.scores.map((scoreItem) => {
          const submission = roundData.submissions.find(
            (sub) => sub.player_id === scoreItem.player_id
          );
          return {
            player_id: scoreItem.player_id,
            name: scoreItem.name,
            similarity: Math.round((scoreItem.match_score ?? 0) * 100),
            time: submission ? submission.remaining_seconds : 0,
            score: Math.round(scoreItem.total_score),
            image_url: submission
              ? `https://scene-hunter-image.yashikota.workers.dev/file/${submission.photo_id}`
              : "",
          };
        });
  
        // スコア順に並び替え
        setPlayers(merged.sort((a, b) => b.score - a.score));
        setLoading(false);
      } catch (err) {
  ...
  return (
    <main className="relative flex flex-col items-center min-h-screen px-4 pt-16 bg-blue-100">
      <Timer seconds={10} onComplete={handleComplete} />
      <h1 className="text-3xl font-bold">Scene Hunter</h1>
      <h2 className="text-xl mt-2">答え合わせ</h2>
      ...
      <div className="mt-4">
        <h3 className="text-md font-semibold text-center mb-2">Game Master</h3>
        <img src={gmImageUrl} alt="GM" className="w-64 h-auto rounded shadow" />
      </div>

      {/* プレイヤー結果 */}
      <div className="w-full max-w-md mt-6">
        <ScrollArea className="h-[300px] border rounded p-4 bg-white">
          <ul className="space-y-4 pr-2">
            {players.map((player) => (
              <li key={player.player_id} className="flex items-center gap-4 justify-center">
                <img
                  src={player.image_url}
                  alt={player.name}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-sm">一致率: {player.similarity}%</p>
                  <p className="text-sm">撮影時間: {player.time}秒</p>
                  <p className="text-sm">得点: {player.score}点</p>
                </div>
              </li>
            ))}
